...
loader = DataLoader(dataset, batch_size=4, shuffle=False, collate_fn=lambda x: x)
...
for i, batch in enumerate(loader):
  for song in batch:
    if song.size(0):
      ...
      print(song.size())
      print(song_enc.size())
      song_dec, _ = vae.decode(song_enc, song.size(0), torch.tensor([dataset.alphabet.index(' ')]))
      print(song_dec.size())
